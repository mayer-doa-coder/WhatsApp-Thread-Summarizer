      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          ...
          <input
            ...
            className="w-full max-w-md bg-slate-900 border border-slate-700 rounded-lg px-4 py-2 text-white placeholder-slate-500 outline-none focus:ring-2 focus:ring-[#25D366]/50 focus:border-[#25D366]/50 transition"
          />
        </div>
        ...
        {!loading && !error && filteredSummaries.length === 0 && (
          <div className="flex flex-col items-center justify-center mt-24 text-center">
            ...
            {!searchTerm && (
              <button
                onClick={() => navigate('/')}
                className="mt-4 rounded-lg bg-[#25D366] px-4 py-2 text-sm font-semibold text-slate-950 transition hover:bg-[#20bc59]"
              >
                Summarize a chat
              </button>
            )}
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right whitespace-nowrap">
                      <button
                        ...
                        className="mr-2 rounded px-3 py-1 text-xs font-medium bg-slate-700 hover:bg-slate-600 text-slate-200 transition"
                      >
                        View
                      </button>
                      <button
                        onClick={() => setDeleteConfirmId(s.id)}
                        className="rounded px-3 py-1 text-xs font-medium bg-red-900/40 hover:bg-red-800/60 text-red-400 hover:text-red-300 border border-red-800/40 transition"
                      >
                        Delete
                      </button>
                    </td>
          ...
          >
            <div className="flex items-start justify-between mb-4 gap-4">
              <div className="min-w-0">
                <h2 className="text-lg font-semibold text-slate-100 truncate">{previewItem.filename}</h2>
              ...
              <button
                ...
                className="flex-shrink-0 text-slate-500 hover:text-slate-200 text-2xl leading-none transition"
              >
                &times;
              </button>
            </div>
      ...
      {deleteConfirmId !== null && (
        ...
        >
          <div className="w-full max-w-sm bg-slate-900 border border-slate-700 rounded-2xl p-6 shadow-2xl">
            <h2 className="text-lg font-semibold text-slate-100 mb-2">Delete summary?</h2>
            ...
            <div className="flex justify-end gap-3">
              <button
                ...
                className="rounded-lg px-4 py-2 text-sm font-medium bg-slate-700 hover:bg-slate-600 text-slate-200 transition"
              >
                Cancel
              </button>
              <button
                onClick={handleConfirmDelete}
                className="rounded-lg px-4 py-2 text-sm font-medium bg-red-700 hover:bg-red-600 text-white transition"
              >
                Confirm Delete
              </button>
            </div>
